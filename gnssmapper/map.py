...
class Map:
    ...
    def is_outside(self,points, polygon=shapely.geometry.Polygon()):
        ...
        if polygon.is_empty:
            ...
            k = [polygon.contains(p) and not self.buildings.intersects(p) for p in mp]
    ...
    def is_los(self,observations):
        ...
        for building,height in zip(self.buildings,self.heights):
            idx= los==True
            n = sum(idx)
            los[idx]=intersects(list(compress(rays_var,idx)),[building]*n,np.ones(n,)*height)
...
def fresnel_parameter(rays,diffraction_points):
    ...
    diffraction_distances = np.array([d.z-p.z for p,d in zip(nearest_points,diffraction_points)]) 
