...
class TestTime(unittest.TestCase):
    def test_gps_utc(self) -> None:
        ...
        pt.assert_extension_array_equal(gps_to_utc(ns).array,ts.array,check_exact=True)
        pt.assert_extension_array_equal(utc_to_gps(ts).array,ns.array,check_exact=True)
  
    def test_gps_doy(self) -> None:
        ns = pd.Series([1,2]).convert_dtypes()
        ts = pd.DataFrame({'date': ['1980006', '1980006'], 'time': [1, 2]}).convert_dtypes()
        pt.assert_frame_equal(gps_to_doy(ns).astype('float64'),ts.astype('float64'),check_exact=True,check_dtype=False)
        pt.assert_extension_array_equal(doy_to_gps(ts.date,ts.time).array,ns.array,check_exact=True)
    ...
    def test_gps_gpsweek(self) -> None:
        ...
        pt.assert_frame_equal(gps_to_gpsweek(ns).astype('float64'),ts.astype('float64'),check_exact=True)
