...
class TestTime(unittest.TestCase):
    ...
    def test_gps_utc_2(self) -> None:
        ns = pd.Series([(1167264018 * 10**9+60*10**9),1167264018*10**9+1+60*60*10**9]).astype(INT_64)
        ts = pd.Series([pd.Timestamp(year=2017,month=1,day=1,hour=0,minute=1,second=0,nanosecond=0),pd.Timestamp(year=2017,month=1,day=1,hour=1,minute=0,second=0,nanosecond=1)])
        pt.assert_extension_array_equal(tm.gps_to_utc(ns).array,ts.array,check_exact=True)
        observed = tm.utc_to_gps(ts).array
        expected = ns.array
        pt.assert_extension_array_equal(observed, expected, check_exact=True)
...
class TestMissing(unittest.TestCase):
    ...
    def test_gps_doy(self) -> None:
        ...
        ts = (pd.DataFrame({'date': ['1980006', '1980006', pd.NA], 'time': [1, 2, pd.NA]})).astype(INT_64)
    ...
    def test_gps_gpsweek(self) -> None:
        ...
        ts = pd.DataFrame({'week':[2000,pd.NA],'day':[0,pd.NA],'time':[1 * 10 ** 7,pd.NA]},dtype=INT_64)
        pt.assert_frame_equal(tm.gps_to_gpsweek(ns).astype('float64'),ts.astype('float64'),check_exact=True)
        pt.assert_extension_array_equal(tm.gpsweek_to_gps(ts.week,ts.day,ts.time).array,ns.array,check_exact=True)
