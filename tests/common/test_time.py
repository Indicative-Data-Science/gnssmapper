...
class TestTime(unittest.TestCase):
    def test_gps_utc(self) -> None:
        ...
        pt.assert_series_equal(gps_to_utc(ns),ts)
        pt.assert_series_equal(utc_to_gps(ts),ns)
  
    def test_gps_doy(self) -> None:
        ns = pd.Series([100,200])
        ts = pd.DataFrame({'date': ['1980006', '1980006'], 'time': [100, 200]})
        print(gps_to_doy(ns))
        pt.assert_frame_equal(gps_to_doy(ns),ts)
        pt.assert_series_equal(doy_to_gps(ts.date,ts.time),ns)
    
    def test_gps_gpsweek(self) -> None:
        ns = pd.Series([604800 * 10**9])
        ts = pd.DataFrame({'week':[1],'day':[0],'time':[0]})
        pt.assert_frame_equal(gps_to_gpsweek(ns),ts)
        pt.assert_series_equal(gpsweek_to_gps(ts.week,ts.day,ts.time),ns)
