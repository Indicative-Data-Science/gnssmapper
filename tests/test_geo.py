...
class TestObservationMethods(unittest.TestCase):
    def setUp(self):
        self.rays = gpd.GeoSeries([shapely.LineString([[527990, 183005, 0], [528020, 183005, 15]]),
    ...
    def test_rays(self) -> None:
        ...
        expected = [shapely.LineString([(0, 0, 0), (1000, 0, 0)]), shapely.LineString([(1, 1, 1), (1001, 1, 1)])]
        out = geo.rays(r, s)
...
class TestFresnel(unittest.TestCase):
    def setUp(self):
        self.buildings = [
            shapely.wkt.loads("POLYGON((528010 183010, 528010 183000,528000 183000, 528000 183010,528010 183010))")]

    def test_fresnel_integral(self):
        v = np.array([-1, 0, 1, 2.4])
        o = np.array([-20 * np.log(1.12), -20 * np.log(0.5), -20 * np.log(0.4 - (0.1184 - 0.28 ** 2) ** 0.5),
                      -20 * np.log(0.225 / 2.4)])
...
class TestMapMethods(unittest.TestCase):
    def setUp(self):
        ...
        self.map_canyon = gpd.GeoDataFrame({'height': [10, 10]},
                                           geometry=[shapely.wkt.loads(
                                               ...
                                               " 528000 183010,528010 183010))"),shapely.wkt.loads("POLYGON((528030 183010, 528030 183000,528020 183000, "
    ...
    def test_is_outside(self):
        ...
        pdt.assert_series_equal(geo.is_outside(self.map_canyon, point_series),
                                pd.Series([False, False], index=[10, 11]), check_names=False)
