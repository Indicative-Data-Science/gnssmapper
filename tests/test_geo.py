...
class TestObservationMethods(unittest.TestCase):
    def setUp(self):
        self.rays = gpd.GeoSeries(
            [
                shapely.LineString([[527990, 183005, 0], [528020, 183005, 15]]),
                shapely.LineString([[527990, 183005, 10], [528020, 183005, 25]]),
            ],
            crs="epsg:27700",
        )
    ...
    def test_rays(self) -> None:
        ...
        expected = [
            shapely.LineString([(0, 0, 0), (1000, 0, 0)]),
            shapely.LineString([(1, 1, 1), (1001, 1, 1)]),
        ]
        out = geo.rays(r, s)
        self.assertTrue(np.all(shapely.predicates.equals(out, expected)))
...
class TestShapelyMethods(unittest.TestCase):
    def setUp(self):
        self.building = shapely.wkt.loads(
    ...
    def test_intersection(self):
        ...
        print(point)
        self.assertAlmostEqual(shapely.get_z(point[0]), 5)

    def test_intersection_projected(self):
        fifteen = shapely.geometry.LineString(
            [[527990, 183005, 10], [528020, 183005, 25]]
        )
        point = geo.intersection_projected([fifteen], [self.building])
        npt.assert_array_almost_equal(
            np.array(list(point)[0].coords).flatten(), [528000, 183005, 15]
        )

        inside = shapely.geometry.LineString(
            [[528005, 183005, 10], [528020, 183005, 25]]
        )
        inside_point = geo.intersection_projected([inside], [self.building])
        npt.assert_array_almost_equal(
            np.array(list(inside_point)[0].coords).flatten(), [528010, 183005, 15]
        )

        outside = shapely.geometry.LineString(
            [[527990, 183015, 10], [528020, 183015, 25]]
        )
    ...
    def test_intersects(self):
        ...
        heights = [10, 10]
        npt.assert_array_almost_equal(
            geo.intersects(rays, buildings, heights), [True, False]
        )
...
class TestMapMethods(unittest.TestCase):
    def setUp(self):
        self.map_box = gpd.GeoDataFrame(
            ...
            geometry=[
                shapely.normalize(
                    shapely.wkt.loads(
                        "POLYGON((528010 183010, 528010 183000,528000 183000,"
        ...
        self.map_canyon = gpd.GeoDataFrame(
            {"height": [10, 10]},
            geometry=[
                shapely.wkt.loads(
                    "POLYGON((528010 183010, 528010 183000,528000 183000,"
                ...
                shapely.wkt.loads(
                    "POLYGON((528030 183010, 528030 183000,528020 183000, "
        ...
        self.rays_box = gpd.GeoSeries(
            [
                shapely.geometry.LineString(
                    [[527990, 183005, 0], [528020, 183005, 15]]
                ),
                shapely.geometry.LineString(
                    [[527990, 183005, 10], [528020, 183005, 25]]
                ),
        ...
        self.rays_canyon = gpd.GeoSeries(
            [
                shapely.geometry.LineString([(527990, 183005, 5), (528015, 183005, 5)]),
    ...
    def test_map_to_crs(self):
        ...
        reverted = reverted.set_geometry(
            shapely.set_precision(reverted.geometry.array.data, 1)
        )
        ...
        pdt.assert_frame_equal(
            self.map_box, reverted, check_dtype=False, atol=0.1, rtol=0.1
        )
    ...
    def test_is_outside(self):
        ...
        point_series = gpd.GeoSeries(
            point.array, crs=self.map_canyon.crs, index=[10, 11]
        )
        pdt.assert_series_equal(
            geo.is_outside(self.map_canyon, point_series),
            pd.Series([False, False], index=[10, 11]),
            check_names=False,
        )
    ...
    def test_is_los(self):
        pdt.assert_series_equal(
            geo.is_los(self.map_box, self.rays_box),
            pd.Series([True, False], index=[1, 2]),
        )
        pdt.assert_series_equal(
            geo.is_los(self.map_canyon, self.rays_canyon),
