...
class TestMapMethods(unittest.TestCase):
    ...
    def test_ground_level(self):
        point = self.map_box.geometry.representative_point()
        npt.assert_array_almost_equal(geo.ground_level(self.map_box,point),[0])

    def test_is_los(self):
        pdt.assert_series_equal(geo.is_los(self.map_box, self.rays_box), pd.Series([True, False]))
        pdt.assert_series_equal(geo.is_los(self.map_canyon, self.rays_canyon), pd.Series([False, False]))

    def test_projected_height(self):  
        a = geo.projected_height(self.map_canyon,self.rays_canyon)
        b = pd.DataFrame(data=[[5,np.nan],[np.nan,9]],columns=[0,1])
