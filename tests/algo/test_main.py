...
from gnssmapper.algo.main import _heights, prepare_data, fit
...
class TestMapAlgorithm(unittest.TestCase):
    ...
    def test_prepare_data(self) -> None:
        expected = pd.DataFrame(data=[[5., np.nan, 1., ], [np.nan, 10., 2.]], columns=[3, 4, 'Cn0DbHz'], index=[1, 2],
                                dtype=np.float64)
    ...
    def test_fit_online(self):
        ...
        params = fit(height, ss, iterations=4, batch_size=3, online=True,
                     starting_params=[[0.8, 1., 2., 0.2], [0.8, 1., 2., 0.2]], online_params={'batch_size': 10})
        ...
        params = fit(height, ss, iterations=2, batch_size=5, online=True,
                     starting_params=[[0.8, 1., 2., 0.2], [0.8, 1., 2., 0.2]], online_params={'batch_size': 3})
    ...
    def test_fit_offline(self):
        ...
        params = fit(data[3], data['Cn0DbHz'], iterations=4, batch_size=80, online=False,
                     starting_params=[[0.8, 1., 2., 0.2], [0.8, 1., 2., 0.2]])
        [npt.assert_almost_equal(params[i - 1, :, :], params[i, :, :]) for i in range(1, 320) if
         i not in {79, 159, 239, 319}]
