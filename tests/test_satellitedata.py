...
class TestSVIDLocation(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.truncated_orbits = cls.example_orbits.loc[
            np.isin(cls.example_orbits["svid"], ["G18", "G14"])
            & (cls.example_orbits["time"] < 2 * 60 * 60 * 10**9)
    ...
    def test_create_orbit(self):
        self.assertCountEqual(
            list(self.SatelliteData.orbits["2020042"].keys()), ["G18", "G14"]
        )
        self.assertEqual(
            len(self.SatelliteData.orbits["2020042"]["G18"]),
            len([3, 7, 11, 15, 19, 20]),
        )
    ...
    def test_locate_sat_check_entries(self):
        entries = pd.Series(
            np.arange(
                np.datetime64("2020-02-11T00:14:42", "ns"),
        ...
        npt.assert_almost_equal(
            list(self.SatelliteData.orbits["2020042"]["G18"]), list(entries["time"])
        )
    ...
    def test_locate_sat(self):
        ...
        predict = self.SatelliteData._locate_sat(
            [gpstime["date"][0]], [gpstime["time"][0]], ["G18"]
        )
        npt.assert_almost_equal(
            predict, np.asarray([entry.x, entry.y, entry.z]).reshape(1, 3), decimal=3
        )
    ...
    def test_locate_satellites(self):
        ...
        entry = self.truncated_orbits.loc[
            (self.truncated_orbits["svid"] == "G18")
            & (self.truncated_orbits["time"] == doy["time"][0]),
            :,
        ].reset_index(drop=True)
        npt.assert_almost_equal(
            np.array(predict.loc[:, "sv_x":"sv_z"]).flatten(),
            np.asarray([entry.x, entry.y, entry.z]).flatten(),
            decimal=0,
        )
...
class TestNameSatellites(unittest.TestCase):
    def test_naming(self) -> None:
        ...
        self.assertSetEqual(set(out.iat[0]), set(list(data.orbits["2020042"].keys())))
