...
# remove the day attribute - it should be calculated from the receiver sample
...
def getSatLocation(sv,times):
    ...
    transmittedTimeNanos = estimateMeasurementTime(times_offset *1e9,sv) #vectorise
    days = times
...
def getSVIDLocation(sp3_df: pd.DataFrame):
    ...
    def locateSatellite(time_nanos, svid):
        time = time_nanos * NANOS_TO_SECOND
        #change it back from nanos?
